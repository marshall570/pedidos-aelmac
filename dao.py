...
class Dao:
    ...
    def insert(self, p):
        try:
            sql_string = f'''INSERT INTO tb_pedidos VALUES (
                {p.code},
                '{p.register_date}',
                '{p.assisted}',
                '{p.need}',
    ...
    def select(self, i):
        try:
            sql_string = f'SELECT * FROM tb_pedidos WHERE ID = {i}'

            conn = db.create_connection()
            cursor = conn.cursor()            
            rs = cursor.execute(sql_string).fetchone()
            return rs
    ...
    def gen_excel(self, p):
        try:
            data = datetime.datetime.now().strftime("%d-%m-%y")
            sql_string = f'SELECT * FROM tb_pedidos WHERE ID >= {p.code}'

            conn = db.create_connection()
            db_df = pd.read_sql_query(sql_string, conn)

            if platform.system() == 'Windows':
                db_df.to_excel(os.path.expanduser("~") + '\\Documents\\Pedidos_AELMAC\\EXCEL\\Pedidos_EXCEL_' + data + '.xlsx', index=False, header=True)
            else:
    ...
    def gen_pdf(self, p):
        try:
            ...
            pdf.write(15, f'PEDIDO DE ORAÇÃO - Nº{resultados[0]} Reg. em {resultados[1]}')
            pdf.ln()
    ...
    def create_directories(self):
        if platform.system() == 'Windows':
            if not os.path.exists(os.path.expanduser("~") + '\\Documents\\Pedidos_AELMAC'):
                os.mkdir(os.path.expanduser("~") + '\\Documents\\Pedidos_AELMAC')

            if not os.path.exists(os.path.expanduser("~") + '\\Documents\\Pedidos_AELMAC\\EXCEL'):
                os.mkdir(os.path.expanduser("~") + '\\Documents\\Pedidos_AELMAC\\EXCEL')

            if not os.path.exists(os.path.expanduser("~") + '\\Documents\\Pedidos_AELMAC\\PDF'):
